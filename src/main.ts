...
class WeatherApp {
  ...
  handleSubmit = async (event: SubmitEvent) => {
    ...
    try {
      const weatherData = await apiManagerForWeather.makeWeatherApiRequest(cityName);
  ...
  handleTyping = () => {
    ...
    const apiManagerForCountry: ApiManager = new ApiManager('', 'https://countriesnow.space/api/v0.1/countries');
    apiManagerForCountry.makeCountryApiRequest().then((cities): void => {
      const filteredCities = cities.filter(city => city.toLowerCase().startsWith(cityPrefix.toLowerCase())).slice(0, 5);
  ...
  showSuggestedCityNameOnUi(filteredCities: CitiesType) {
    const listElements = filteredCities.map(cityName => this.generateSuggestionLiTemplate(cityName));
    this.suggestionsElement && (this.suggestionsElement.innerHTML = listElements.join(''));

    const suggestedLiElements: NodeListOf<HTMLLIElement> | undefined = this.suggestionsElement?.querySelectorAll('.search-bar__suggestion');
  ...
  updateUiWithWeatherData(weatherData: WeatherDataType) {
    ...
    this.weatherDetailsElement && (this.weatherDetailsElement.textContent = description);
    this.weatherImgElement && (this.weatherImgElement.style.backgroundImage = `url(${imageUrl})`);
    ...
    this.temperatureElement && (this.temperatureElement.textContent = Math.round(temp).toString());
